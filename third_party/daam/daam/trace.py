...
class DiffusionHeatMapHooker(AggregateHooker):
    def __init__(
        ...
        self.latent_hw = latent_h**2 # 4096 if h == 512 else 1024 # 9216  # 64x64 or 96x96 depending on if it's 2.0-v or 2.0
        print(f'Latent HW: {self.latent_hw}')
    ...
    def compute_global_heat_map(self, prompt=None, factors=None, head_idx=None, layer_idx=None, normalize=False):
        ...
        return GlobalHeatMap(self.pipe.tokenizer, prompt, maps, self.maps_bkp)
...
class PipelineHooker(ObjectHooker[StableDiffusionPipeline]):
    ...
    def _hooked_run_safety_checker(hk_self, self: StableDiffusionPipeline, image, *args, **kwargs):
        ...
        hk_self.parent_trace.last_image = pil_image

        return image, has_nsfw

    def _hooked_encode_prompt(hk_self, _, prompt: Union[str, List[str]], *args, **kwargs): # StableDiffusionPipeline
